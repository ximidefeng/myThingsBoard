...
public class BaseEntityService extends AbstractEntityService implements EntityService {
    ...
    public PageData<EntityData> findEntityDataByQuery(TenantId tenantId, CustomerId customerId, EntityDataQuery query) {
        if (query.getTemp() != null && query.getTemp()) {
            customerId = new CustomerId(CustomerId.NULL_UUID);
        }
        if(StringUtils.isNotEmpty(query.getTempDeviceName())){
            Device device = deviceService.findDeviceByTenantIdAndName(tenantId, query.getTempDeviceName());
            if (device == null) {
                throw new IncorrectParameterException("Incorrect tempDeviceName " + query.getTempDeviceName());
            } else {
                if (query.getEntityFilter() instanceof SingleEntityFilter) {
                    SingleEntityFilter entityFilter = (SingleEntityFilter) query.getEntityFilter();
                    entityFilter.setSingleEntity(device.getId());
                } else {
                    throw new IncorrectParameterException("Incorrect entityType " + query.getEntityFilter().getType());
                }
            }
        }
