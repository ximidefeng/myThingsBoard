...
public class DefaultEntityQueryRepository implements EntityQueryRepository {
    ...
    public PageData<EntityData> findEntityDataByQuery(TenantId tenantId, CustomerId customerId, EntityDataQuery query) {
        if(StringUtils.isNotEmpty(query.getTempDeviceName())&& query.getTemp()){
            return findTempEntityDataByQuery(tenantId, customerId, query, false);
        }
        return findEntityDataByQuery(tenantId, customerId, query, false);
    }

    private PageData<EntityData> findTempEntityDataByQuery(TenantId tenantId, CustomerId customerId, EntityDataQuery query, boolean ignorePermissionCheck) {
        return transactionTemplate.execute(status -> {
            EntityType entityType = resolveEntityType(query.getEntityFilter());
            ...
            String entityWhereClause = DefaultEntityQueryRepository.this.buildTempEntityWhere(ctx, query.getEntityFilter(), entityFieldsFiltersMapping);
            String latestJoinsCnt = EntityKeyMapping.buildLatestJoins(ctx, query.getEntityFilter(), entityType, allLatestMappings, true);
    ...
    private String buildTempEntityWhere(QueryContext ctx, EntityFilter entityFilter, List<EntityKeyMapping> entityFieldsFilters) {
        String permissionQuery = this.defaultPermissionQuery(ctx);
        String entityFilterQuery = this.singleEntityQuery(ctx, (SingleEntityFilter) entityFilter);
