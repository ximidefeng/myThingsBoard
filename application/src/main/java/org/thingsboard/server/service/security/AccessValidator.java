...
public class AccessValidator {
    ...
    public void validate(SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback, boolean temp) {
        switch (entityId.getEntityType()) {
            case DEVICE:
                validateDevice(currentUser, operation, entityId, callback, temp);
                return;
            case DEVICE_PROFILE:
    ...
    private void validateDevice(final SecurityUser currentUser, Operation operation, EntityId entityId, FutureCallback<ValidationResult> callback, boolean temp) {
            ...
            Futures.addCallback(deviceFuture, getCallback(callback, device -> {
                if (device == null) {
                    ...
                    if(device.getCustomerId().equals(customerId)){
                        ...
                        // 那就去关系表中查询是否是下属用户
                        List<EntityRelationInfo> entityRelationInfos = findAgentRelationsByCustomerId(currentUser.getTenantId(), customerId);
                        AtomicBoolean isToUser = new AtomicBoolean(false);
                        for (EntityRelationInfo entityRelationInfo : entityRelationInfos) {
                            UUID otherId = entityRelationInfo.getTo().getId();
                            if (device.getCustomerId().equals(new CustomerId(otherId))) {
                                isToUser.set(true);
                                break;
                            }
                        }
