...
import org.thingsboard.server.utils.ResponseUtil;
...
public class DeviceController extends BaseController {
    ...
    @RequestMapping(value = "/tx/customer/{customerId}/device/{deviceName}", method = RequestMethod.POST)
    @ResponseBody
    public Object assignDeviceToCustomerV2(@PathVariable("customerId") String strCustomerId,
                                         @PathVariable("deviceName") String deviceName) throws ThingsboardException {
        checkParameter("customerId", strCustomerId);
        checkParameter("deviceName", deviceName);
        CustomerId customerId = new CustomerId(toUUID(strCustomerId));
        Customer customer = customerService.findCustomerById(customerId);
        Device device = checkNotNull(deviceService.findDeviceByTenantIdAndName(customer.getTenantId(), deviceName));
        if(!device.getCustomerId().isNullUid()){
            throw new ThingsboardException("device is binding", ThingsboardErrorCode.DEVICE_ALREADY_BINDING);
        }
        return ResponseUtil.ok(tbDeviceService.assignDeviceToCustomer(customer.getTenantId(), device.getId(), customer));
    }
    ...
    @RequestMapping(value = "/tx/tenant/devices", params = {"tenantId", "deviceName"}, method = RequestMethod.GET)
    @ResponseBody
    public Device getTxTenantDevice(
            @RequestParam String tenantId,
            @RequestParam String deviceName) throws ThingsboardException {
        try {
            return checkNotNull(deviceService.findDeviceByTenantIdAndName(TenantId.fromUUID(UUID.fromString(tenantId)), deviceName));
        } catch (Exception e) {
            throw handleException(e);
        }
    }
