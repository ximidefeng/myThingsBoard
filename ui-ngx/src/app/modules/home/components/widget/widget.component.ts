...
export class WidgetComponent extends PageComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    let tempDeviceName = this.route.snapshot.queryParamMap.get('tempDeviceName') ?? ""
    if (tempDeviceName.length > 0) {
      this.entityService.setupTempDeviceName(tempDeviceName)
      this.entityDataService.setupTempDeviceName(tempDeviceName)
    }
