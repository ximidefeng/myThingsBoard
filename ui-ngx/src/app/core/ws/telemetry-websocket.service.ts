...
export class TelemetryWebsocketService implements TelemetryService {
  ...
  public setupTempDeviceName(deviceName: string) {
    ...
    subscriber.subscriptionCommands.forEach(
      (subscriptionCommand) => {
        ...
        if (subscriptionCommand instanceof SubscriptionCmd) {
          ...
          //1111111111
          if (this.tempDeviceName && this.tempDeviceName.length > 0) {
            subscriptionCommand.temp = true;
            subscriptionCommand.tempDeviceName = this.tempDeviceName;
          }
