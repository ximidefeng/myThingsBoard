...
export class MobileService {
  ...
  private doDashboardNavigation(openDashboardMessage: OpenDashboardMessage) {
    ...
    if (openDashboardMessage.isTemp) {
      params.push(`temp=true`);
    }
    if (openDashboardMessage.tempDeviceName && openDashboardMessage.tempDeviceName.length > 0) {
      params.push(`tempDeviceName=${openDashboardMessage.tempDeviceName}`);
    }
